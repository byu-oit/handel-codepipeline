...
function getSourcePhase(phaseSpec, githubAccessToken) {
    winston.info(`Creating source phase '${phaseSpec.name}'`);
...
function getBuildPhase(sourcePhaseSpec, phaseSpec, handelFile) {
    winston.info(`Creating build phase '${phaseSpec.name}'`);
    if (!sourcePhaseSpec) {
...
function getDeployPhase(buildPhaseSpec, phaseSpec, handelFile) {
    winston.info(`Creating deploy phase '${phaseSpec.name}'`);
    if (!buildPhaseSpec) {
...
function createPipeline(codePipeline, accountId, pipelineDefinition, handelFile, accountConfig, githubAccessToken, region) {
    validatePipelineDefinition(pipelineDefinition);

    let codePipelineBucketName = `codepipeline-${region}-${accountId}`
    let projectName = handelFile.name;
    return s3Calls.createBucketIfNotExists(codePipelineBucketName, region)
        .then(bucket => {
...
exports.createPipelines = function (pipelinesToAccountsMapping, handelCodePipelineFile, handelFile, accountConfigs, githubAccessToken, region) {
    ...
    for (let pipelineName in handelCodePipelineFile.pipelines) {
        ...
        let createPipelinePromise = createPipeline(codePipeline, accountId, pipelineDefinition, handelFile, accountConfig, githubAccessToken, region)
            .then(pipeline => {
