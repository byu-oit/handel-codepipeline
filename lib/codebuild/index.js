...
function createBuildPhaseCodeBuildProject(accountId, projectName, buildPhase, region) {
    winston.info(`Creating build phase CodeBuild project ${projectName}`);
    return createBuildPhaseServiceRole(accountId)
        .then(buildPhaseRole => {
            let buildProjectName = codeBuildCalls.getBuildProjectName(projectName);
            return codeBuildCalls.createProject(buildProjectName, projectName, buildPhase.build_image, buildPhase.environment_variables, accountId, buildPhaseRole.Arn, region);
        });
}

function createDeployPhaseCodeBuildProject(accountId, projectName, deployPhase, accountConfig, region) {
    ...
    return createDeployPhaseServiceRole(accountId)
        .then(deployPhaseRole => {
            ...
            return codeBuildCalls.createProject(deployProjectName, projectName, handelDeployImage, handelDeployEnvVars, accountId, deployPhaseRole.Arn, region, handelDeployBuildSpec);
        });
}

function createCodeBuildProjectsInAccount(accountId, projectName, pipelineDefinition, accountConfig, region) {
    let createPromises = [];
    ...
    for(let i = 2; i < pipelineDefinition.phases.length; i++) {
        ...
        if(phase.type === 'handel') {
            createPromises.push(createDeployPhaseCodeBuildProject(accountId, projectName, phase, accountConfig, region));
        }
    }
    
    //Create build phase project
    let buildPhase = pipelineDefinition.phases[1];
    createPromises.push(createBuildPhaseCodeBuildProject(accountId, projectName, buildPhase, region));

    return Promise.all(createPromises);
}

exports.createCodeBuildProjects = function(pipelinesToAccountsMapping, handelCodePipelineFile, handelFile, accountConfigs, region) {
    ...
    for(let pipelineName in handelCodePipelineFile.pipelines) {
        ...
        let createProjectPromise = createCodeBuildProjectsInAccount(accountId, handelFile.name, pipelineDefinition, accountConfig, region)
            .then(project => {
