...
function getDeployProjectName(handelFileName, envsToDeploy) {
...
function createDeployPhaseCodeBuildProject(phaseContext, accountConfig) {
    let handelFileName = phaseContext.handelAppName;
    let deployProjectName = getDeployProjectName(handelFileName, phaseContext.params.environments_to_deploy)
            ...
            return codeBuildCalls.getProject(deployProjectName)
                .then(buildProject => {
                    if (!buildProject) {
                        ...
                        return codeBuildCalls.createProject(deployProjectName, handelFileName, handelDeployImage, handelDeployEnvVars, phaseContext.accountConfig.account_id, deployPhaseRole.Arn, phaseContext.accountConfig.region, handelDeployBuildSpec);
                    }
                    else {
                        winston.info(`Updating Handel deploy phase CodeBuild project ${deployProjectName}`);
                        return codeBuildCalls.updateProject(deployProjectName, handelFileName, handelDeployImage, handelDeployEnvVars, phaseContext.accountConfig.account_id, deployPhaseRole.Arn, phaseContext.accountConfig.region, handelDeployBuildSpec)
                    }
                });
        });
}
        ...
        actions: [
            {
                ...
                configuration: {
                    ProjectName: getDeployProjectName(phaseContext.handelAppName, phaseContext.params.environments_to_deploy)
                },
                runOrder: 1
            }
        ]
...
exports.deletePhase = function (phaseContext, accountConfig) {
    ...
    let codeBuildProjectName = getDeployProjectName(phaseContext.handelAppName, phaseContext.params.environments_to_deploy);
