            ...
            return codeBuildCalls.getProject(buildProjectName)
                .then(buildProject => {
                    if (!buildProject) {
                        ...
                        winston.info(`Updating build phase CodeBuild project ${handelAppName}`);
                        return codeBuildCalls.updateProject(buildProjectName, handelAppName, phaseContext.params.build_image, phaseContext.params.environment_variables, phaseContext.accountConfig.account_id, buildPhaseRole.Arn, phaseContext.accountConfig.region)
                    }
                });
        });
}
...
exports.deletePhase = function (phaseContext, accountConfig) {
    ...
    let codeBuildProjectName = getBuildProjectName(phaseContext.handelAppName);
