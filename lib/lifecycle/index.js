...
exports.getPhaseSecrets = function (phaseDeployers, handelCodePipelineFile, pipelineName, args) {
    return new Promise((resolve, reject) => {
        let inputFunctions = [];

        for (let phaseSpec of handelCodePipelineFile.pipelines[pipelineName].phases) {
            let phaseType = phaseSpec.type;
            if(args && args.secrets) {
                let results = [];
                let phaseName = phaseSpec.name;
                args.secrets.phases.forEach(phase => {
                    if (phase.name === phaseName) {
                        phase.secrets.forEach(secret => {
                            let secretObj = {};
                            secretObj[secret.name] = secret.value
                            results.push(secretObj)
                        })
                    }
                })
