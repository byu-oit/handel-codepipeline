...
describe('s3 calls module', () => {
    ...
    describe('createBucket', () => {
        it('should create the bucket', async () => {
            ...
            const getBucketStub = sinon.stub(s3Calls, 'getBucket').returns(Promise.resolve({
                Name: bucketName
            }));
    ...
    describe('createBucketIfNotExists', () => {
        it('should create the bucket if it doesnt exist', async () => {
            const bucketName = 'FakeBucket';
            const getBucketStub = sinon.stub(s3Calls, 'getBucket').returns(Promise.resolve(null));
            const createBucketStub = sinon.stub(s3Calls, 'createBucket').returns(Promise.resolve({
                Name: bucketName
            }));
